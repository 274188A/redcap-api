...
namespace Redcap
{
    public partial class RedcapApi
    {
        ...
        ///
        /// </example>
        public async Task<string> ExportUserRolesAsync(Content content = Content.UserRole, RedcapFormat format = RedcapFormat.json, RedcapReturnFormat returnFormat = RedcapReturnFormat.json, CancellationToken cancellationToken = default, long timeOutSeconds = 100)
        {
        ...
        public async Task<string> ImportUserRolesAsync<T>(List<T> data, Content content = Content.UserRole, RedcapFormat format = RedcapFormat.json, RedcapReturnFormat returnFormat = RedcapReturnFormat.json, CancellationToken cancellationToken = default, long timeOutSeconds = 100)
        {
            return await ExecuteAsync(payload =>
            {
                AddFormattedRequest(payload, content, format, returnFormat);
        ...
        public async Task<string> DeleteUserRolesAsync(List<string> roles, Content content = Content.UserRole, RedcapAction action = RedcapAction.Delete, CancellationToken cancellationToken = default, long timeOutSeconds = 100)
        {
            return await ExecuteAsync(payload =>
            {
        ...
        public async Task<string> ExportUserRoleAssignmentAsync(Content content = Content.UserRoleMapping, RedcapFormat format = RedcapFormat.json, RedcapReturnFormat onErrorFormat = RedcapReturnFormat.json, CancellationToken cancellationToken = default, long timeOutSeconds = 100)
        {
            return await ExecuteAsync(payload =>
            {
        ...
        public async Task<IReadOnlyList<RedcapUserRoleAssignment>> ExportUserRoleAssignmentTypedAsync(Content content = Content.UserRoleMapping, RedcapReturnFormat onErrorFormat = RedcapReturnFormat.json, CancellationToken cancellationToken = default, long timeOutSeconds = 100)
        {
            ...
            try
            {
                var assignments = JsonConvert.DeserializeObject<List<RedcapUserRoleAssignment>>(response);
                if (assignments == null)
                {
                    throw new RedcapApiException("REDCap returned an empty user role assignment payload.");
                }
