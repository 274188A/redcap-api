...
public partial class RedcapApi
{
    ...
    public async Task<string> ExportUsersAsync(RedcapFormat format = RedcapFormat.json, RedcapReturnFormat returnFormat = RedcapReturnFormat.json, CancellationToken cancellationToken = default, long timeOutSeconds = 100)
    {
        return await ExecuteAsync(payload =>
        {
            AddFormattedRequest(payload, Content.User, format, returnFormat);
    ...
    public async Task<string> ImportUsersAsync<T>(List<T> data, RedcapFormat format = RedcapFormat.json, RedcapReturnFormat returnFormat = RedcapReturnFormat.json, CancellationToken cancellationToken = default, long timeOutSeconds = 100)
    {
        return await ExecuteAsync(payload =>
        {
            AddFormattedRequest(payload, Content.User, format, returnFormat);
    ...
    public async Task<string> DeleteUsersAsync(List<string> users, Content content = Content.User, RedcapAction action = RedcapAction.Delete, CancellationToken cancellationToken = default, long timeOutSeconds = 100)
    {
        return await ExecuteAsync(payload =>
        {
            AddActionRequest(payload, content, action);
            AddIndexedValues(payload, "users", users);
        }, cancellationToken, timeOutSeconds);
    }
