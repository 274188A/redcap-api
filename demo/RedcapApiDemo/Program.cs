...
namespace RedcapApiDemo
{
    partial class Program
    {
        static async Task Main(string[] args)
        {
             ...
             * 3. A download folder is created automatically at <UserProfile>/redcap_download_files.
             *    Override via the DownloadPath setting in appsettings.Development.json or the
             *    REDCAP_DEMO_DOWNLOAD_PATH environment variable.
             ...
             *
             * CLI flags:
             *  -m / --method <name>    Run a single demo method and exit.
             *  -n / --non-interactive  Skip all prompts; use config defaults. Also auto-detected
             *                          when stdin is redirected or REDCAP_DEMO_NON_INTERACTIVE=1.
             */
            InitializeInteractiveMode(args);
        ...
        static async Task InitializeDemo(string methodToRun = null)
        {
            ...
            // Provide your REDCap project/super tokens via appsettings.Development.json or the REDCAPDEMO_* env vars.
            // These placeholders are not valid REDCap tokens; the demo will not authenticate until you supply real values.
            var defaultProjectToken = string.IsNullOrWhiteSpace(devSettings.ProjectToken) ? "REPLACE_ME_PROJECT_TOKEN" : devSettings.ProjectToken.Trim();
            string _token = defaultProjectToken;
            string _superToken = string.IsNullOrWhiteSpace(devSettings.SuperToken)
                ? "REPLACE_ME_SUPER_TOKEN"
            ...
            */
            ...
            var filedDownloadPath = ResolveDownloadPath(devSettings);
            ...
            if(!localArtifactsOk)
            {
                ...
                var continueWithoutArtifacts = ReadInput();
            ...
            var ExportLoggingAsync = await redcap_api.ExportLoggingAsync(_token, RedcapFormat.json, LogType.User);
            ...
            var ImportDagsAsyncResult = await redcap_api.ImportDagsAsync(_token, RedcapFormat.json, dags);
            ...
            var ExportDagsAsyncResult = await redcap_api.ExportDagsAsync(_token, RedcapFormat.json);
            ...
            var DeleteDagsAsyncResult = await redcap_api.DeleteDagsAsync(_token, dagsToDelete);
            ...
            var ImportRecordsAsync = await redcap_api.ImportRecordsAsync(_token, RedcapFormat.json, RedcapDataType.flat, OverwriteBehavior.normal, forceAutoNumber: true, backgroundProcess: false, importDemographicsData, "MDY", CsvDelimiter.tab, ReturnContent.ids, RedcapReturnFormat.json);
            ...
            var ExportRecordsAsyncResult = await redcap_api.ExportRecordsAsync(_token, RedcapFormat.json, RedcapDataType.flat, recordsToExport, null, instrumentName);
            ...
            var DeleteRecordsAsync = await redcap_api.DeleteRecordsAsync(_token, recordsToExport, 1);
            ...
            else
            {
                ...
                var newRecordName = "2";
                var RenameRecordAsyncResult = await redcap_api.RenameRecordAsync(_token, recordToRename, newRecordName, 1);
            ...
            var randomizationResult = await redcap_api.RandomizeRecord(_token, record, randomizationId, RedcapFormat.json, RedcapReturnFormat.json);
            ...
            var ExportArmsAsyncResult = await redcap_api.ExportArmsAsync(_token, RedcapFormat.json, arms, RedcapReturnFormat.json);
            ...
            var ImportArmsAsyncResult = await redcap_api.ImportArmsAsync(_token, Override.False, RedcapAction.Import, RedcapFormat.json, ImportArmsAsyncData, RedcapReturnFormat.json);
            ...
            var DeleteArmsAsyncResult = await redcap_api.DeleteArmsAsync(_token, DeleteArmsAsyncData);
            ...
            var ExportEventsAsyncResult = await redcap_api.ExportEventsAsync(_token, RedcapFormat.json, ExportEventsAsyncData, RedcapReturnFormat.json);
            ...
            var ImportEventsAsyncResult = await redcap_api.ImportEventsAsync(_token, Override.False, RedcapFormat.json, eventList, RedcapReturnFormat.json);
            ...
            var DeleteEventsAsyncResult = await redcap_api.DeleteEventsAsync(_token, DeleteEventsAsyncData);
            ...
            var SwitchDagAsyncResult = await redcap_api.SwitchDagAsync(_token, SwitchDagAsyncData);
            ...
            var ExportFieldNamesAsyncResult = await redcap_api.ExportFieldNamesAsync(_token, RedcapFormat.json, "first_name", RedcapReturnFormat.json);
            ...
            var ImportFileAsyncResult = await redcap_api.ImportFileAsync(_token, recordId, fieldName, eventName, null, fileName, filePath, RedcapReturnFormat.json);
            ...
            var ExportFileAsyncResult = await redcap_api.ExportFileAsync(_token, recordId, fieldName, eventName, null, RedcapReturnFormat.json);
            ...
            var ExportFileAsyncResult2 = await redcap_api.ExportFileAsync(_token, recordId, fieldName, eventName, null, RedcapReturnFormat.json, filedDownloadPath);
            ...
            var DeleteFileAsyncResult = await redcap_api.DeleteFileAsync(_token, recordId, fieldName, eventName, "1", RedcapReturnFormat.json);
            ...
            var ExportInstrumentsAsyncResult = await redcap_api.ExportInstrumentsAsync(_token, RedcapFormat.json);
            ...
            var ExportPDFInstrumentsAsyncResult = await redcap_api.ExportPDFInstrumentsAsync(_token, recordId, eventName, "demographics", true, (string?)null);
            ...
            var additionalWiringResponse = ReadInput();
            ...
            var ImportInstrumentMappingAsyncResult = await redcap_api.ImportInstrumentMappingAsync(_token, RedcapFormat.json, importInstrumentMappingData, RedcapReturnFormat.json);
            ...
            var ExportMetaDataAsyncResult = await redcap_api.ExportMetaDataAsync(_token, RedcapFormat.json, null, null, RedcapReturnFormat.json);
            ...
            var CreateProjectAsyncResult = await redcap_api.CreateProjectAsync(_superToken, RedcapFormat.json, projectData, RedcapReturnFormat.json, null);
            ...
            var ImportProjectInfoAsyncResult = await redcap_api.ImportProjectInfoAsync(_token, RedcapFormat.json, projectInfo);
            ...
            var ExportProjectInfoAsyncResult = await redcap_api.ExportProjectInfoAsync(_token, RedcapFormat.json);
