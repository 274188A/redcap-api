...
5. Local download folder is writable. By default the demo uses
   `<UserProfile>/redcap_download_files` and creates it on first run.
   Override via the `DownloadPath` setting in
   `appsettings.Development.json` or the `REDCAP_DEMO_DOWNLOAD_PATH`
...
`DownloadPath` is optional — when empty the demo writes to
`<UserProfile>/redcap_download_files`.
...
- `REDCAP_DEMO_DOWNLOAD_PATH`
- `REDCAP_DEMO_NON_INTERACTIVE` (set to `1`/`true`/`yes` to skip all prompts and auto-accept defaults)
